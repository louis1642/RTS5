...
void * sensor_driver_loop(void* par) {
	...
	mqd_t sensor_qd;
	if ((sensor_qd = mq_open(SENSOR_L_QUEUE_NAME, O_WRONLY | O_CREAT, QUEUE_PERMISSIONS, &attr)) == -1) {
...
void * actuator_driver_loop(void* par) {
	...
	mqd_t actuator_qd;
	if ((actuator_qd = mq_open(ACTUATOR_L_QUEUE_NAME, O_RDONLY | O_CREAT, QUEUE_PERMISSIONS, &attr)) == -1) {
...
int main(void) {
	...
	if (mq_unlink (SENSOR_L_QUEUE_NAME) == -1) {
        perror("Main: mq_unlink sensor queue");
        exit(1);
    }

	if (mq_unlink (ACTUATOR_L_QUEUE_NAME) == -1) {
        perror("Main: mq_unlink actuator queue");
        exit(1);
    }

 	printf("The plant L STOPPED\n");
	return 0;
}
