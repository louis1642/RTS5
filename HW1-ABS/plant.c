...
void * plant_loop(void * par) {
	...
	while (keep_on_running)
	{
		...
		if (count%2 == 0) {
			...
			if (shared_actuator.value == 0) {
				brake = 0; skate = 0;
			}
			else if (shared_actuator.value == 1) {
				sensor++; brake = 0; skate = 0;
			}	
			else if ((shared_actuator.value == -1) && (sensor>0)) {
				sensor--; brake = 0; skate = 0;
			}	
			else if ((shared_actuator.value== -2) && (sensor>0)) { 
				// emula frenata e skating
				brake++;
				if (brake < 10) { // all'inizio la frenata funziona
					if(sensor>2) sensor-=3; else sensor=0;
